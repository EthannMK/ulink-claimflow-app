...
export function SlaPage() {
  const [rows, setRows] = useState<Sla[]>(() => mockSla.map((r) => ({ ...r })))
  ...
  function saveEdit() {
    ...
    setRows((rs) => rs.map((r, j) => j === editing ? draft : r)); setEditing(null)
  }
  function addRow() {
    if (!draft.category.trim()) return
    setRows((rs) => [...rs, draft]); setDraft({ category: '', target: '', warn: '', breach: '' }); setAdding(false)
  }
          ...
          <tbody>
            {adding && (
              <tr className="border-t border-outline-variant bg-primary/[0.03]">
                ...
                <td className="px-4 py-2 text-right whitespace-nowrap"><Button size="sm" onClick={addRow}>Save</Button></td>
              </tr>
            )}
            {rows.map((r, i) => editing === i ? (
              <tr key={i} className="border-t border-outline-variant bg-primary/[0.03]">
                ...
                <td className="px-4 py-2 text-right whitespace-nowrap"><Button size="sm" onClick={saveEdit}>Save</Button></td>
              </tr>
            ) : (
